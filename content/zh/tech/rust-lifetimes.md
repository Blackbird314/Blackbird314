...
生命周期(Lifetimes)是 Rust 独有的一个概念，借用检查器用它来避免悬垂引用。生命周期具有许多微妙之处：
...
Rust 实际上不允许命名具体的生命周期类型，上述 `&'r i32` `&'a str` 不是合法的语法。一般来说，不涉及函数调用时，Rust 可以自行推断所需的生命周期，只有跨越函数的边界传递引用，才需要手动标注生命周期。
...
当生命周期涉及到函数调用时，情况会复杂一点，譬如借用检查如何避免返回悬垂引用？答案是比较函数签名返回值和实际返回值的生命周期：
...
上例中 `transfer_str()` 函数的定义是正确的，但函数调用的结果被赋值给具有 `'static` 生命周期的 `r`，这要求本次调用的 `'a` 单态化为 `'static`，显然与 `argument` 的生命周期矛盾。

生命周期分析还是借用规则的基础。

## 子类型与型变

生命周期是类型的一部分，那么赋值过程也会涉及到生命周期的传递：Subtyping
