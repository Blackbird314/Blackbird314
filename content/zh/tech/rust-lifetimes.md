...
在此情形下，我们计算 `lvalue` 的 *Supporting prefixes* 集合，并寻找集合中所有解引用 `*lv`（假设 `lv` 的生命周期为 `'a`）；随后添加约束 `('a: 'b) @ P`，其中 `P` 为借用开始生效的节点。
...
截至目前，我们仅讨论了函数作用域内的借用问题，编译器可以自动推理相关生命周期。当跨越函数的边界传递引用时，需要开发者显式标注生命周期。除 `'static` 外，Rust 允许以 `<'r>` 语法命名..泛型生命周期..。每次函数调用或类型实例化，`'r` 都会单态化(monomorphization)为一个具体的代码区间。
