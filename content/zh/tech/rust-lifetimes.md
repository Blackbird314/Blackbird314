...
这里说的“保持有效”，是针对引用的地址值，而非持有它的变量或表达式，变量仍需遵守作用域规则（所谓“非词法生命周期”）。多数情况下，变量作用域和引用的生命周期重合，因为编译器..总是试图最小化生命周期的范围..；但变量离开作用域后，它持有的引用仍可能保持有效，这时引用的生命周期也可被延长至超过变量作用域：
...
乍一看有点反直觉，但正如 `Cat` 拥有 `Animal` 的所有属性和方法，`'long` 也包含了 `'short` 定义的全部区域。本质上，子类型是在超类型的基础上拓展得来，它比超类型具有更多的“内涵”。

Rust 支持生命周期约束，语法 `'a: 'b` 的意思是 `'a` outlives `'b`，这代表着 `'a` <: `'b`。
