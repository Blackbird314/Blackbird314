...
“引用保持有效”，意思是指向的对象没有被移动或销毁，也没有违反借用规则，而持有引用的变量或表达式仍遵守“非词法作用域”规则。多数情况下，变量作用域和引用的生命周期重合，因为编译器..总是试图最小化生命周期的范围..；但变量离开作用域后，它持有的引用仍可能保持有效，这时引用的生命周期可以比变量作用域更长：
...
`'static` 即所谓静态生命中，是一个容易被误解的概念，它限定的对象从创建到程序结束都是有效的，并不一定要在编译时创建。因为字符串字面值 `World` 在程序运行期间始终存在，故 `r2` 可以标注为 `'static`。
...
第一条生命周期原则很直观，若一个引用的生命周期为 `'a`，那么它指向对象的生存期必然不小于 `'a`，所以编译器不允许将 `r3` 标注为 `'static` 生命周期：
...
`<'a>` 声明了一个..泛型生命周期..，每次调用 `return_str` 时，都会单态化(monomorphization)得到一个具体的生命周期，在这个范围内返回引用保持有效。`'a` 是一个外部的生命周期，其真实范围由..调用者..决定，而实际返回的表达式 `&s`，其生命周期被局限在函数内部，显然不能满足 `&'a str`，于是借用检查报错。
...
下面这个例子以内存泄漏为代价，得到了一个具有 `'static` 生命周期的可变引用 `r`，接着对 `r` 做两次重借用。按照非词法作用域分析，这不会违反借用规则。但对同一对象借可变引用 `r2`，会截断 `r1` 的生命周期，与 `'static` 标注矛盾，导致报错。
...
`test` 函数签名实际为 `impl Copy + Display + use<'a, T>`，这要求 `'a` 包含返回值的使用范围。因为每次调用返回的类型可能不同，如 `i32`、`&i32` 或其它类型，而编译器统一视为满足 `'a` 约束的 `impl Copy + Display`。为了避免悬垂引用，`'a` 至少要包括打印语句，显然与 `&num1` 的生命周期矛盾。

若将函数签名改为 `fn test<'a, T: Copy + Display>(para: &'a T) -> T` 则不会报错，因为编译器会将 `num2` 的类型 `T` 单态化为 `i32`，不会对返回值施加 `'a` 限制。

## 高阶特型约束

高阶特型约束(HRTBs)的全称是 _Higher-Rank Trait Bounds_。
