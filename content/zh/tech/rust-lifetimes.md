...
生命周期分析基于以控制流图(CFG)表示的 [MIR](https://blog.rust-lang.org/2016/04/19/MIR.html)，而非以抽象语法树(AST)表示的 HIR。具体来说，生命周期被定义为 MIR 控制流图中一系列节点的集合；在下文中，我们会进一步扩展这个定义，以涵盖 "Skolemized" 生命周期 —— 对应于函数定义中的泛型生命周期参数。
...
截至目前，我们仅讨论了函数作用域内的借用问题，编译器可以自动推理相关生命周期。当跨越函数的边界传递引用时，需要开发者显式标注有关生命周期。除 `'static` 和 `'_` 外，Rust 只允许标注以 `<'a>` 语法命名的..泛型生命周期..。每次函数调用或类型实例化，`'a` 都会单态化(monomorphization)为一个具体的代码区间。

函数签名和变量类型涉及的泛型生命周期（如 `'a`），被定义为以下要素的集合：

- 当前函数的全部节点 
- `end('a)` —— 调用者在函数调用后的部分节点

<!-- 我们必须调整子类型化的定义以适应修正后的区域定义。具体而言，`('a: 'b) @ P` 的约束被扩展为： -->
