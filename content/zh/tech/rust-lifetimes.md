...
`&'static` 表示引用从创建到程序结束都必须有效。因为字符串字面值 `World` 在程序运行期间始终存在，故 `r2` 可以标注为 `'static`。
...
下面这个例子以内存泄漏为代价，得到了一个具有静态生命周期的可变引用 `r`，接着对 `r` 做两次重借用。按照非词法作用域分析，这不会违反借用规则。但对同一对象借可变引用 `r2`，会截断 `r1` 的生命周期，与 `'static` 标注矛盾，导致报错。
...
什么样的 `T` 对 `'a` 有效？如果类型 `T` 是引用或包含引用，那么 `T` 所有引用的生命周期必须 outlive `'a`，以保证在 `'a` 内不会出现悬垂引用；如果类型 `T` 不含引用，例如 `i32` `Box<i32>` `String`，则自动满足约束 —— 这些类型永远不必担心悬垂引用。

按照上述规则，所有 `&'a T` 类型都满足 `T: 'a` 约束，而 `&'a T` 要求并隐含了 `T: 'a`。

```Rust
```

`'static` 是一个容易误解的概念，它限定的对象从创建到程序结束都是有效的，而不一定
