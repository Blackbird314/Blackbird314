...
生命周期分析基于以控制流图(CFG)表示的 [MIR](https://blog.rust-lang.org/2016/04/19/MIR.html)，而非以抽象语法树(AST)表示的 HIR。具体来说，生命周期被定义为 MIR 控制流图中一系列节点的集合，如果生命周期包含点 `P`，说明引用在 `P` 点处有效；在下文中，我们会进一步扩展这个定义，以涵盖 "Skolemized" 生命周期 —— 对应于函数声明中的具名生命周期。
...
在此情形下，我们计算 `lvalue` 的 _Supporting prefixes_ 集合，并寻找集合中所有解引用 `*lv`（假设 `lv` 的生命周期为 `'a`）；随后添加约束 `('a: 'b) @ P`，其中 `P` 为借用开始生效的节点。
...
形如 `('a: 'b) @ P` 的约束条件意味着：从点 `P` 出发，生命周期 `'a` 必须包含 `'b` 中所有可从 `P` 到达的点。具体实现时，编译器从 `P` 点开始对 `'b` 进行[深度优先搜索]()，通过遍历 CFG 中可能的代码路径，将搜索到的每个有效节点添加至 `'a` 集合中；若搜索过程超出生命周期 `'b` 范围，则退出该条路径的搜索。例如，本例中从 `A1` 可以到达 `B0` 和 `C0`，却不能到达 `B3` `B4`（`'p` 存在空洞，搜索到 `B1` 节点则停止搜索 `if` 块）。
...
截至目前，我们仅讨论了函数作用域内的借用问题，编译器可以自动推理相关生命周期。当跨越函数的边界传递引用时，需要开发者显式标注生命周期。除 `'static` 外，Rust 允许以 `<'r>` 语法命名..泛型生命周期..。每次函数调用或类型实例化，`'r` 都会单态化(_monomorphization_)为一个具体的代码区间。

特别的，函数定义涉及的具名生命周期（如 `'r`）被定义为至少包含以下要素的集合：

- 当前函数 CFG 的全部节点
- `end('r)` —— 函数返回后调用者（或其调用者等）的某些节点
...
`'static` 被称为静态生命周期，语义是..有效期持续到程序结束..。和 `'a` 类似，`'static` 包括一个表示为 `end('static)` 的元素，对应函数返回后程序执行的剩余部分。

字符串字面值在程序运行期间始终有效，所以它的引用类型可以是 `&'static str`。很多人存在一种误解，认为 `'static` 引用的对象必须在编译时创建且不可变。但以内存泄漏为代价，我们可以得到指向运行时内存的 `&'static mut T` 引用：
...
函数声明实际上将代码分成了两个部分：函数定义和函数调用。借用检查不会跨函数进行分析，它首先检查函数定义，然后检查函数调用，考虑下例：
...
我们希望 Rust 拒绝这个程序，理由是共享引用 `x` 指向 `data` 的一个子集，而我们试图对 `data` 本身借可变引用，这违反了借用规则。但借用检查器并不理解“子集”这个概念，它对索引操作解糖后看到的是 `Index::index` 函数调用：

```Rust
let mut data: Vec<i32> = vec![1, 2, 3];
'x: {
    let x: &i32 = Index::index(&data, 0); // 'x 如我们所愿地小（刚好够 println!）
    'a: {
...
`x` 的生命周期被推断为 `'x`，根据方法签名和子类型化约束，入参 `&data` 的生命周期也被推断为 `'x`，这与生命周期为 `'a` 的 `&mut data` 冲突，于是报错。
...
生命周期的子类型化引入了一个新的问题：若 `'sub` <: `'super`，那么对于生命周期构造出的类型 `F<'a>`，也应该有 `F<'sub>` :< `F<'super>` 吗？回答这个问题，首先要理解型变的概念。
