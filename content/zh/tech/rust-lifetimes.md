...
截至目前，我们仅讨论了函数作用域内的借用问题，编译器可以自动推理相关生命周期。当跨越函数的边界传递引用时，需要开发者显式标注生命周期。除编译器保留的 `'static` 外，Rust 还允许以 `<'r>` 语法命名..泛型生命周期..。每次函数调用或类型实例化，`'r` 都会单态化(monomorphization)为一个具体的代码区间。

函数声明涉及的泛型生命周期（如 `'r`）为至少包含以下要素的集合：

- 当前函数 CFG 的全部节点
- `end('r)` —— 调用者在函数返回后的某些节点的集合
