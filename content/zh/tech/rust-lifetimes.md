...
截至目前，我们的讨论局限在函数作用域内，编译器会自动推断这些生命周期。只有跨越函数的边界传递引用时，才需要开发者显示标注生命周期。除 `'static` 和 `'_` 外，Rust 只能标注以 `<'a>` 语法声明的..泛型生命周期..。每次函数调用或结构体实例化，都会单态化(monomorphization)确定一个具体的生命周期。
