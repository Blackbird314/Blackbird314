...
生命周期分析基于以控制流图(_CFG_)表示的 [MIR](https://blog.rust-lang.org/2016/04/19/MIR.html)，而非以抽象语法树(_AST_)表示的 HIR。具体来说，生命周期被定义为 MIR 控制流图中一系列节点的集合，如果生命周期包含点 `P`，说明引用在 `P` 点处有效；在下文中，我们会进一步扩展这个定义，以涵盖 "_Skolemized_" 生命周期 —— 对应于函数定义中的具名生命周期。
