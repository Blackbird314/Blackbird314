...
`'static` 标注的引用从创建到程序结束都必须保持有效，字符串字面量 `World` 在程序运行期间始终存在，故 `r2` 可以拥有 `'static` 生命周期。
...
第一条原则很直观，若一个引用的生命周期为 `'a`，那么它指向对象的生存期必然不小于 `'a`，所以编译器不允许将 `r3` 标注为 `'static` 生命周期：
...
let r1: &'static str = &*r; // 删掉 'static 即可
let r2 = &mut *r;
```

<!-- 对生命周期的一个常见的误解是 static， -->

Rust 支持生命周期约束语法，`'a: 'b` 的意思是 `'a` outlives `'b`，这代表着 `'a` <: `'b`。
...
可问题在于，Rust 中 `'a` 并不是一种单独的类型，它只能作为类型的一部分出现。若有 `'long` <: `'short`，那么对于生命周期构造出的类型，如 `F<'a>`，也应该有 `F<'long>` :< `F<'short>` 吗？回答这个问题，首先要理解型变的概念。
