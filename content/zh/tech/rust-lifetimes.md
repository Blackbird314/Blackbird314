...
截至目前，我们仅讨论了函数作用域内的借用问题，编译器可以自动推理相关生命周期。当跨越函数的边界传递引用时，需要开发者显式标注生命周期。除 `'static` 外，Rust 还允许以 `<'r>` 语法声明..泛型生命周期..。每次函数调用或类型实例化，`'r` 都会单态化(_monomorphization_)为一个具体的代码区间。
...
还有一个编译器内置的静态生命周期 `'static`，语义是..到程序结束保持有效..。与 `'a` 类似，`'static` 包括一个表示为 `end('static)` 的元素，对应当前函数返回后程序执行的剩余部分。
