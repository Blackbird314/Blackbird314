...
`'static` 标注的引用从创建到程序结束都是“有效的”。字符串字面量 `World` 在程序运行期间始终存在，故 `r2` 可以拥有 `'static` 生命周期。
...
下面这个例子以内存泄漏为代价，得到了一个具有静态生命周期的可变引用 `r`，接着对 `r` 做两次重借用。按照非词法作用域分析，这不会违反借用规则。但对同一对象借新的可变引用 `r2`，会截断 `r1` 的生命周期，与 `'static` 标注矛盾，导致报错。
...
至此，我们已经讨论完了标准库提供的类型。结构体、枚举和联合体类型的型变性取决于其字段的型变性。如果一个泛型参数被用于具有不同型变性的字段，那么该参数只能是不变的。例如，以下结构体对于 `'a` 和 `T` 是协变的，而对于 `'b`、`'c` 和 `U` 则是不变的：
